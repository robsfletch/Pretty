program pretty_loghist
	version 13
	syntax varlist [if] [in] [, unsure(int -888) refusal(int -555) ///
	...
	***************************
	...
	qui sum `varlist' if `touse' & !inlist(`varlist', `unsure', `refusal', ///
								 `other', `na', 0)
	local max = `r(max)'
	local min = `r(min)'
	...
	*/
	...
	graph twoway bar Probs4 Probs1 if Probs3 >= `min', scheme(pretty1) ///
